...
function CopyBtn({ text }: { text: string }) {
  ...
  return (
    <button onClick={() => { copyToClipboard(text); setCopied(true); setTimeout(() => setCopied(false), 1500) }}
...
export default function EyedropperPanel() {
  ...
  async function pickColor() {
    ...
    try {
      ...
      const color: PickedColor = { hex: hex.toUpperCase(), rgb: `rgb(${r}, ${g}, ${b})`, hsl: hexToHsl(hex), id: `${Date.now()}-${Math.random()}` }
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="px-3 pt-3 pb-2 flex gap-2">
        <button onClick={pickColor} disabled={isPicking}
      ...
      {colors.length === 0 ? (
        <div className="flex flex-col items-center justify-center flex-1 gap-3 px-4 text-center">
          ...
          <p className="text-xs" style={{ color: S.sub }}>Click "Pick Color" then click<br />anywhere on the page</p>
        </div>
      ) : (
            ...
            <div className="flex flex-wrap gap-2">
              {colors.map((color) => (
                <div key={color.id} className="relative group">
                  <button onClick={() => setActiveId(color.id)} title={color.hex}
                  ...
                  <button onClick={() => deleteColor(color.id)} title="Remove"
