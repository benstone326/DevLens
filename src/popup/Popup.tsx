...
// Extracted so each card has its own useHover — can't call hooks inside .map()
function ToolCard({ tool, onClick }: { tool: typeof TOOLS[0]; onClick: () => void }) {
  ...
  return (
    <button
      ...
      style={{
        background:   'Field',
        border:      `1px solid ${hovered ? `${tool.color}55` : 'color-mix(in srgb, ButtonBorder 35%, Canvas)'}`,
  ...
  return (
    <div style={{ width: '300px', background: 'Canvas', color: 'CanvasText', colorScheme: 'light dark' }}>
      <div className="flex items-center gap-2.5 px-4 py-3"
           style={{ borderBottom: '1px solid color-mix(in srgb, ButtonBorder 35%, Canvas)' }}>
